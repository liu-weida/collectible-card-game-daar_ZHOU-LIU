...
contract Main is Ownable {
    ...
    /**
     * @dev 兑换一个 Booster，用户用它换取其中包含的卡牌，但需要确认才能最终转移所有权
     * @param boosterTokenId Booster 的 Token ID
     */
    function redeemBooster(uint256 boosterTokenId) external {
        ...
        pendingTransfers[boosterTokenId] = msg.sender;
    ...
    /**
     * @dev 确认兑换，并在确认后转移卡包的所有权
     * @param boosterTokenId Booster 的 Token ID
     */
    function confirmRedeem(uint256 boosterTokenId) external {
        address redeemer = pendingTransfers[boosterTokenId];
        require(redeemer == msg.sender, "Main: Only the redeemer can confirm");


        Boosters boostersInstance = boosters[0];
        boostersInstance.transferFrom(redeemer, owner(), boosterTokenId);
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            ...
            collection.mint(redeemer, cardNumber, imgURI);
        }


        delete pendingTransfers[boosterTokenId];
    }
