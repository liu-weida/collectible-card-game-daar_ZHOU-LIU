...
contract Boosters is ERC721, Ownable {
    ...
    uint256 public collectionId; // 关联的 Collection ID

    // 物品的详细信息，包括卡牌编号和图片 URI
    struct CardInfo {
        uint256 cardNumber;
        string imgURI;
    }

    // 每个 Booster 中包含的物品（Booster ID => CardInfo 数组）
    mapping(uint256 => CardInfo[]) public boosterContents;

    mapping(uint256 => string) public imgURIs;           // tokenId => 图片 URI
    mapping(uint256 => ItemDetails) public itemDetails;  // tokenId => 物品详细信息
    ...
    function cancelSale(uint256 tokenId) external {
        ...
        itemDetails[tokenId].price = -1;  // 恢复为非出售状态
